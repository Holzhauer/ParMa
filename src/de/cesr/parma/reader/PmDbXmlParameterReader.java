...
/**
 * Reads the database settings from an XML-File (specified by
 * PmFrameworkPa.DB_SETTINGS_FILE)
 * 
...
public class PmDbXmlParameterReader extends PmAbstractParameterReader {
	...
	static private Logger logger = Logger
			.getLogger(PmDbXmlParameterReader.class);
	 ...
	 * @param settingsFile
	 *            parameter definition of location of XML file that specifies
	 *            database settings
	 */
	public PmDbXmlParameterReader(PmParameterManager pm,
			PmParameterDefinition settingsFile) {
		// <- LOGGING
		if (logger.isDebugEnabled()) {
			logger.debug("Init PmDbXmlParameterReader with "
					+ PmParameterManager.getFullName(settingsFile));
		}
		// LOGGING ->
	...
	public PmDbXmlParameterReader() {
		this(PmFrameworkPa.DB_SETTINGS_FILE);
		// <- LOGGING
		if (logger.isDebugEnabled()) {
			logger.debug("Init PmDbXmlParameterReader with "
					+ PmParameterManager
							.getFullName(PmFrameworkPa.DB_SETTINGS_FILE));
		}
		// LOGGING ->
	}
	...
	public void initParameters() {
		...
		logger.info("Read Database settings from XML-File "
				+ pm.getParam(settingsFile));
		File file = new File((String) pm.getParam(settingsFile));

		if (!file.exists()) {
			logger.warn("DB Settings XML file ("
					+ pm.getParam(this.settingsFile)
					+ ") does not exist! "
			...
			try {
				...
				if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					NodeList fstNmElmntLst = fstElmnt
							.getElementsByTagName("SQL_LOCATION");
					Element fstNmElmnt = (Element) fstNmElmntLst.item(0);
					NodeList fstNm = fstNmElmnt.getChildNodes();
					pm.setParam(PmFrameworkPa.LOCATION,
							(fstNm.item(0)).getNodeValue());

					NodeList dbnameNmElmntLst = fstElmnt
							.getElementsByTagName("SQL_DBNAME");
					Element dbnameNmElmnt = (Element) dbnameNmElmntLst.item(0);
					NodeList dbnameNm = dbnameNmElmnt.getChildNodes();
					pm.setParam(PmFrameworkPa.DBNAME,
							(dbnameNm.item(0)).getNodeValue());

					NodeList userNmElmntLst = fstElmnt
							.getElementsByTagName("SQL_USER");
					Element userNmElmnt = (Element) userNmElmntLst.item(0);
					NodeList userNm = userNmElmnt.getChildNodes();
					pm.setParam(PmFrameworkPa.USER,
							(userNm.item(0)).getNodeValue());

					NodeList pwNmElmntLst = fstElmnt
							.getElementsByTagName("SQL_PASSWORD");
					Element pwNmElmnt = (Element) pwNmElmntLst.item(0);
					NodeList pwNm = pwNmElmnt.getChildNodes();
					pm.setParam(PmFrameworkPa.PASSWORD,
							(pwNm.item(0)).getNodeValue());

					NodeList tbNmElmntLst = fstElmnt
							.getElementsByTagName("SQL_TBLNAME_PARAMS");
					Element tbNmElmnt = (Element) tbNmElmntLst.item(0);
					NodeList tbNm = tbNmElmnt.getChildNodes();
					pm.setParam(PmFrameworkPa.TBLNAME_PARAMS,
							(tbNm.item(0)).getNodeValue());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
